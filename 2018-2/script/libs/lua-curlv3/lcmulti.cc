...
static int lcurl_opt_set_long_(lua_State *L, int opt){
  ...
  code = curl_multi_setopt(p->curl, (CURLMoption)opt, val);
  if(code != CURLM_OK){
    return lcurl_fail_ex(L, p->err_mode, LCURL_ERROR_MULTI, code);
  }
  lua_settop(L, 1);
...
static int lcurl_opt_set_string_array_(lua_State *L, int opt){
  ...
  if(n == 0){
    code = curl_multi_setopt(p->curl, (CURLMoption)opt, 0);
  }
  else{
    int i;
    char const**val = (const char**)malloc(sizeof(char*) * (n + 1));
    if(!val){
...
)
{
  ...
  curl_multi_setopt(p->curl, (CURLMoption)OPT_CB, (c->cb_ref == LUA_NOREF)?0:func);
  curl_multi_setopt(p->curl, (CURLMoption)OPT_UD, (c->cb_ref == LUA_NOREF)?0:p);
...
int lcurl_multi_timer_callback(CURLM *multi, long ms, void *arg){
  lcurl_multi_t *p = (lcurl_multi_t*)arg;
  lua_State *L = p->L;
  int n, top, ret = 0;
...
static int lcurl_multi_set_TIMERFUNCTION(lua_State *L){
  ...
  return lcurl_multi_set_callback(L, p, &p->tm,
    ...
    "timer", (void*)lcurl_multi_timer_callback
  );
}
...
static int lcurl_multi_set_SOCKETFUNCTION(lua_State *L){
  ...
  return lcurl_multi_set_callback(L, p, &p->sc,
    ...
    "socket", (void*)lcurl_multi_socket_callback
  );
}
