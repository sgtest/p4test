...
static int lcurl_opt_set_long_(lua_State *L, int opt){
  ...
  code = curl_easy_setopt(p->curl, (CURLoption)opt, val);
  if(code != CURLE_OK){
    return lcurl_fail_ex(L, p->err_mode, LCURL_ERROR_EASY, code);
  }
  lua_settop(L, 1);
...
static int lcurl_opt_set_slist_(lua_State *L, int opt, int list_no){
  ...
  code = curl_easy_setopt(p->curl, (CURLoption)opt, list);
...
static int lcurl_easy_set_STREAM_DEPENDS_impl(lua_State *L, int opt){
  ...
  CURLcode code = curl_easy_setopt(p->curl, (CURLoption)opt, e->curl);
  if(code != CURLE_OK){
    return lcurl_fail_ex(L, p->err_mode, LCURL_ERROR_EASY, code);
  }
...
static int lcurl_opt_unset_slist_(lua_State *L, int opt, int list_no){
  ...
  code = curl_easy_setopt(p->curl, (CURLoption)opt, NULL);
...
static int lcurl_info_get_long_(lua_State *L, int opt){
  ...
  code = curl_easy_getinfo(p->curl, (CURLINFO)opt, &val);
  if(code != CURLE_OK){
    return lcurl_fail_ex(L, p->err_mode, LCURL_ERROR_EASY, code);
  }
...
)
{
  ...
  code = curl_easy_setopt(p->curl, (CURLoption)OPT_CB, (c->cb_ref == LUA_NOREF)?0:func);
  ...
  curl_easy_setopt(p->curl, (CURLoption)OPT_UD, (c->cb_ref == LUA_NOREF)?0:p);
...
static size_t lcurl_hpost_read_callback(char *buffer, size_t size, size_t nitems, void *arg){
  lcurl_hpost_stream_t *p = (lcurl_hpost_stream_t*)arg;
  assert(NULL != p->L);
  return lcurl_read_callback(*p->L, &p->rd, &p->rbuffer, buffer, size, nitems);
}
...
static int lcurl_easy_set_DEBUGFUNCTION(lua_State *L){
  ...
  return lcurl_easy_set_callback(L, p, &p->debug,
    ...
    "debug", (void*)lcurl_debug_callback
  );
}
